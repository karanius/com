...
import { SecretPasswordContainerLocal } from "coral-admin/__generated__/SecretPasswordContainerLocal.graphql";
...
import styles from "./SecretPasswordContainer.css";

// eslint-disable-next-line no-unused-expressions
graphql`
  fragment SecretPasswordContainer_formValues on Auth {
    integrations {
      local {
        enabled
        allowRegistration
        showCommentStreamRegistration
        targetFilter {
          admin
          stream
        }
...
const SecretPasswordContainer: FunctionComponent<Props> = ({ disabled }) => {
  const [{ forceAdminLocalAuth }] =
    useLocal<SecretPasswordContainerLocal>(graphql`
      fragment SecretPasswordContainerLocal on Local {
        forceAdminLocalAuth
      }
    `);
...
export default SecretPasswordContainer;

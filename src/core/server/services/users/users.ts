...
export async function phoneAlreadyExists(
  mongo: MongoContext,
  tenant: Tenant,
  username: string
) {
  return !!(await retrieveUserByPhone(mongo, tenant.id, username));
}
...
export async function emailAlreadyExists(
  ...
  return !!(await retrieveUserByEmail(mongo, tenant.id, email));
}

...
export interface User extends TenantResource {
  ...
  /**
   * Phone number is for a specific User.
   */
  phoneNumber?: string;
  ...
  /**
   * phoneVerified when true indicates that the given email address has been verified.
   */
...
export async function retrieveUserByPhone(
  mongo: MongoContext,
  tenantID: string,
  phoneNumber: string
) {
  return mongo.users().findOne({ tenantID, phoneNumber });
}
...
export async function retrieveUserByEmail(
  ...
  return mongo.users().findOne({ tenantID, email });
}
...
export async function retrieveUserWithPhone(
  mongo: MongoContext,
  tenantID: string,
  phoneNumber: string
) {
  return mongo.users().findOne({
    ...
    $or: [
      {
        profiles: {
          $elemMatch: { id: phoneNumber, type: "local" },
        },
      },
      { phoneNumber },
    ],
...
export async function createOrRetrieveUserEmailVerificationID(
  ...
  if (!result.value) {
    ...
    if (user.phoneVerified) {
      throw new Error("Phone number has already been verified");
    }
